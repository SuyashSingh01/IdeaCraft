...
exports.isWriter=async function(req,res,next){
    try {
        if (req.user.role !== 'Writer'&&req.user.role !== 'Admin') {
            return res.status(403).json({
                success: false,
                message: "You are not allowed to access this Writer route"
            })
        }
        next();
...
exports.isRecuriter = async (req, res, next) => {
    try {
        if (req.user.role !== 'Recuriter') {
            return res.status(403).json({
                success: false,
                message: "You are not allowed to access this Recuriter route"
            })
        }
        next();
